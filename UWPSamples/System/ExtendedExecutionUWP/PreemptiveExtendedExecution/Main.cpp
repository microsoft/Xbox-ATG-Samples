...
//
// This file has been modified to facilitate the Extended Execution sample.
//
...
public:
    ...
    virtual void Initialize(CoreApplicationView^ applicationView)
    {
        ...
        m_session = ref new ExtendedExecutionSession();
        //This must be set to either Unspecified or LocationTracking otherwise the extension request will be denied for a preemptive request.
        m_session->Reason = ExtendedExecutionReason::Unspecified;
        m_session->Description = "Testing Extended Execution behavior with a preemptive request.";
        m_session->Revoked += ref new TypedEventHandler<Platform::Object^, ExtendedExecutionRevokedEventArgs^>(this, &ViewProvider::ExtensionRevokedHandler);

        m_sample = std::make_unique<Sample>(m_session);
...
protected:
    ...
    void OnActivated(CoreApplicationView^ applicationView, IActivatedEventArgs^ args)
    {
        ...
        m_sample->LogEvent(L"OnActivated()", args->Kind.ToString()->Data());
    ...
    void OnSuspending(Platform::Object^ sender, SuspendingEventArgs^ args)
    {
        if (m_sample->GetUseDeferral())
        {
            ...
            {
                m_sample->LogEvent(L"OnSuspending()", L"using a deferral");
        ...
        {
            m_sample->LogEvent(L"OnSuspending()", L"not using a deferral");
            m_sample->OnSuspending();
        }
    }
    ...
    {
        m_sample->LogEvent(L"OnResuming()");
        m_sample->OnResuming();
    }
    ...
    void OnVisibilityChanged(CoreWindow^ sender, VisibilityChangedEventArgs^ args)
    {
        ...
        if (m_visible)
        {
            m_sample->LogEvent(L"OnVisibilityChanged()", L"Visible");
            m_sample->OnActivated();
        }
        else
        {
            m_sample->LogEvent(L"OnVisibilityChanged()", L"Not Visible");
    ...
    void ExtensionRevokedHandler(Platform::Object^ obj, ExtendedExecutionRevokedEventArgs^ args)
    {
        if (args->Reason == ExtendedExecutionRevokedReason::Resumed)
        {
            m_sample->LogEvent(L"Extension Revoked.", L"Reason = Resumed");
        }
        else if (args->Reason == ExtendedExecutionRevokedReason::SystemPolicy)
        {
            m_sample->LogEvent(L"Extension Revoked.", L"Reason = SystemPolicy");
        }
    }
...
private:
    ...
    Windows::ApplicationModel::ExtendedExecution::ExtendedExecutionSession^ m_session;
