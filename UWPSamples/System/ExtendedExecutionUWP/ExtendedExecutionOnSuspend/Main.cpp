...
protected:
    ...
    void OnSuspending(Platform::Object^ sender, SuspendingEventArgs^ args)
    {
        ...
        concurrency::create_task([this, deferral]()
        {
            ...
            //This must be set to SavingData otherwise the extension request will be denied during a suspension.
            session->Reason = ExtendedExecutionReason::SavingData;
            session->Description = "Testing Extended Execution behavior with a request during suspension.";
            session->Revoked += ref new TypedEventHandler<Platform::Object^, ExtendedExecutionRevokedEventArgs^>(this, &ViewProvider::ExtensionRevokedHandler);

            m_sample->LogEvent(L"OnSuspending()", L"using a deferral");
            m_sample->OnSuspending(session);
            deferral->Complete();
        });
    }
    ...
    {
        m_sample->SetExtensionActive(false);
