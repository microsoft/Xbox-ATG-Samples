...
// File: TexConv.cpp
//
// DirectX Texture Converter for Xbox One
//
...
#pragma prefast(disable : 26018, "Only used with static internal arrays")
    ...
    void PrintLogo()
    {
        wprintf(L"Microsoft (R) DirectX Texture Converter for Xbox One\n");
...
int __cdecl wmain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[])
{
    ...
    for (int iArg = 1; iArg < argc; iArg++)
    {
        ...
        if (('-' == pArg[0]) || ('/' == pArg[0]))
        {
            ...
            case OPT_XGMODE:
                ...
                {
                    static const SValue s_pXGModes[] =
                    {
                    ...
                    DWORD mode = LookupByName(pValue, s_pXGModes);
                    if (!mode)
                    {
    ...
    for (auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv)
    {
        ...
        // --- Save result -------------------------------------------------------------
        {
            ...
            PrintInfo(info, (FileType == CODEC_DDS) && (dwOptions & (DWORD64(1) << OPT_USE_XBOX)));
            wprintf(L"\n");
            ...
            {
            case CODEC_DDS:
                if (dwOptions & (DWORD64(1) << OPT_USE_XBOX))
                {
                    Xbox::XboxImage xbox;
                    ...
                    if (SUCCEEDED(hr))
                    {
                        hr = Xbox::SaveToDDSFile(xbox, pConv->szDest);
                    }
                }
                else
                {
