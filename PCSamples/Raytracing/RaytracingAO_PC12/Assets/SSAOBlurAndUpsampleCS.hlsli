...
ConstantBuffer<BlurAndUpscaleConstantBuffer> BlurAndUpscaleCB : register(b1);
...
{
    float temp = d1 * d2 + BlurAndUpscaleCB.stepSize;
    return temp * temp > l1 * l2 * BlurAndUpscaleCB.blurTolerance;
}
...
void BlurHorizontally(uint leftMostIndex)
{
    ...
    float l01 = d01 * d01 + BlurAndUpscaleCB.stepSize;
    float l12 = d12 * d12 + BlurAndUpscaleCB.stepSize;
    float l23 = d23 * d23 + BlurAndUpscaleCB.stepSize;
    float l34 = d34 * d34 + BlurAndUpscaleCB.stepSize;
    float l45 = d45 * d45 + BlurAndUpscaleCB.stepSize;
    float l56 = d56 * d56 + BlurAndUpscaleCB.stepSize;
...
{
    float4 weights = float4(9, 3, 1, 3) / (abs(HiDepth - LowDepths) + BlurAndUpscaleCB.upsampleTolerance);
    float TotalWeight = dot(weights, 1) + BlurAndUpscaleCB.noiseFilterStrength;
    float WeightedSum = dot(LowAO, weights) + BlurAndUpscaleCB.noiseFilterStrength;
...
void main(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    //
    PrefetchData(GTid.x << 1 | GTid.y << 5, int2(DTid.xy + GTid.xy - 2) * BlurAndUpscaleCB.invLowResolution);
    ...
    // 39 is used since (39 / 3) -> 13 columns (verticle) and (GI % 3) * 3 -> 9 values per row (horizontal)
    //
    if (GI < 39)
    ...
    // 39 is used since (45 / 9) * 2 -> 10 columns (verticle) and (GI % 9) -> 10 values per row (horizontal)
    // One extra value is produced and not used
    //
    if (GI < 45)
    ...
    float2 UV0 = DTid.xy * BlurAndUpscaleCB.invLowResolution;
    float2 UV1 = DTid.xy * 2 * BlurAndUpscaleCB.invHighResolution;
